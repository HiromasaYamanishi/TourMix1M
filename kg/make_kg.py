...
def preprocess_user_df():
   """
   ユーザーレビューのDataFrameから名詞ペアと形容詞-名詞ペアを抽出して保存
   Args:
       user_df: user_id, reviewを含むDataFrame
   """
   user_df = pd.read_csv('../data/kg/user_review_fix.csv')
   pandarallel.initialize(progress_bar=True, nb_workers=32)
   
   # 形容詞-名詞ペアと名詞-名詞ペアの抽出
   user_df['pairs_adj'] = user_df['review'].parallel_apply(find_adj_noun_pairs)
   user_df['pairs_noun'] = user_df['review'].parallel_apply(find_noun_pairs)
   
   # 抽出結果の保存
   with open('../data/kg/user_pairs_adj.pkl', 'wb') as f:
       pickle.dump(user_df['pairs_adj'], f)
   
   with open('../data/kg/user_pairs_noun.pkl', 'wb') as f:
       pickle.dump(user_df['pairs_noun'], f)
       
   print("Extracted and saved user review pairs")
   return user_df['pairs_adj'], user_df['pairs_noun']

def preprocess_spot_df():
