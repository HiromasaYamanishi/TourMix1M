...
class LazySupervisedDataset(Dataset):
    ...
    def retrieve_triplets(self, entity, prompt_ind, neighbor_thresh=20):
        triplets = []
        ...
        for entity_ind, neighbor in enumerate(neighbors):
            edge_data = self.sakg[entity][neighbor]['relations']
            sorted_edge_data = sorted(edge_data.items(), key=lambda x: x[1], reverse=True)[:20]        
            triplets.extend([(entity, neighbor, relation, count, prompt_ind, entity_ind) for relation, count in sorted_edge_data])
        return triplets
